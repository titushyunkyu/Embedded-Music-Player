...
static void SPI_OutData(uint8_t data)
{
    LCD_RS_PORT->DOUTSET31_0 = LCD_RS_MASK; // RS = PA13 = 1 for data
    SPISendByte(data);
}

static void SPI_OutCommand(uint8_t command)
{
    LCD_RS_PORT->DOUTCLR31_0 = LCD_RS_MASK; // RS = PA13 = 0 for command
    SPISendByte(command);
...
void static commonInit(const uint8_t *cmdList) {
  ...
  IOMUX->SECCFG.PINCM[LCD_RS_INDEX] = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE1);
  IOMUX->SECCFG.PINCM[LCD_RST_INDEX] = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE1);

  ClockDelay(POWER_STARTUP_DELAY);

  // Enable the outputs
  LCD_RS_PORT->DOE31_0 |= LCD_RS_MASK;      // LCD RS
  LCD_RST_PORT->DOE31_0 |= LCD_RST_MASK;    // LCD !RST

  // Initialize to inactive values.
  LCD_RS_PORT->DOUTSET31_0 = LCD_RS_MASK;   // RS = 1
  LCD_RST_PORT->DOUTSET31_0 = LCD_RST_MASK; // !RST = 1

  SPI_Config SPI1Config =
  {
   .SCK_index = PB9INDEX,
   .SCK_iomuxMode = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE3),
   .PICO_index = PB8INDEX,
   .PICO_iomuxMode = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE3),
   .POCI_index = PB7INDEX,
   .POCI_iomuxMode = (IOMUX_PINCM_INENA_ENABLE | IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE3),
   .CS_index = PB6INDEX,
   .CS_iomuxMode = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE3),
   .divideRatio = SPI_CLKDIV_RATIO_DIV_BY_1,
   .scr = 0,
   .ctl0 = (SPI_CTL0_FRF_MOTOROLA_4WIRE | SPI_CTL0_DSS_DSS_8),
   .ctl1 = (SPI_CTL1_MSB_ENABLE | SPI_CTL1_CP_ENABLE | SPI_CTL1_ENABLE_ENABLE)
  };
  InitializeSPI(SPI1,&SPI1Config);

  // toggle RST low to reset; CS low so it'll listen to us
  LCD_RST_PORT->DOUTSET31_0 =  LCD_RST_MASK; // PB15=!RST=1
  ClockDelay_1ms(500);        // 500ms (calibrated with logic analyzer)
  LCD_RST_PORT->DOUTCLR31_0 = LCD_RST_MASK; // PB15=!RST=0
  ClockDelay_1ms(500);        // 500ms
  LCD_RST_PORT->DOUTSET31_0 = LCD_RST_MASK; // PB15=!RST=1
  ClockDelay_1ms(500);        // 500ms
...
// (SPI PICO), J2.17 (LCD ~RST), and J4.31 (LCD DC).
// Input: none
// Output: none
void LCD_Init(void){
